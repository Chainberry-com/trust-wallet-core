...
Both platforms pin **Trust Wallet Core 4.1.19** (`com.trustwallet:wallet-core:4.1.19` in `android/build.gradle`; `s.dependency 'TrustWalletCore', '4.1.19'` in the podspec). Address derivation vectors for Ethereum, BNB, and Polygon are verified in `conformance/address-derivation-vectors.json` and asserted by both the Android instrumented test (`src/androidTest/.../AddressDerivationConformanceTest.kt`; run via `./gradlew connectedDebugAndroidTest`) and the iOS conformance test (`ios/ConformanceTests/AddressDerivationConformanceTests.swift`; add to an XCTest target in vault.xcworkspace to run). Address derivation for Bitcoin, Litecoin, XRP, Tron, TON, Solana, and Bitcoin Cash is **pending on-device verification** — run the instrumented test on a real device with 4.1.19 and promote the results to `status: "verified"` in the fixture. Signing vectors for all chains are pending harvest into `conformance/signing-vectors.json` — confirm each against a testnet broadcast or the TWC reference suite before promoting.
